...
import { productCategories, formatCurrency, customerProfile } from '../lib/mockData'
            ...
            <nav className={styles.navbar}>
                ...
                <div className={styles.tabNav}>
                    {[
                        ...
                        { id: 'cart', label: cart.length ? `Cart (${cart.length})` : 'Cart' },
                        { id: 'orders', label: myOrders.length ? `Orders (${myOrders.length})` : 'Orders' },
                        { id: 'wishlist', label: wishlist.length ? `Wishlist (${wishlist.length})` : 'Wishlist' },
                        { id: 'profile', label: 'Profile' }
                    ].map(t => (
                        <button key={t.id} className={`${styles.tabBtn} ${activeTab === t.id ? styles.tabBtnActive : ''}`} onClick={() => setActiveTab(t.id)}>
                            {t.label}
                        </button>
                    ))}
                </div>
